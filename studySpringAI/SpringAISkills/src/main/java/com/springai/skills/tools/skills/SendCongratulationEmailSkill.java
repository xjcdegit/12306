...
/**
 * 发送祝贺邮件技能 - 复杂 Skill 示例实现
 * 
 * <p>SendCongratulationEmailSkill 是一个复杂的工作流 Skill，
 * 封装了"查询最高分用户并发送祝贺邮件"的完整业务流程。</p>
 * 
 * <h3>IoC 设计说明：</h3>
 ...
 */
@Component
public class SendCongratulationEmailSkill implements Skill {

    private final ToolRegistry toolRegistry;

    public SendCongratulationEmailSkill(ToolRegistry toolRegistry) {
        this.toolRegistry = toolRegistry;
    }

    @Override
    public String getName() {
        return "send_congratulation_email";
    }

    @Override
    public String getDescription() {
        return "Query highest score user and send congratulation email";
    }

    @Override
    public List<String> getRequiredTools() {
        return List.of("query_database", "send_email");
    }
    ...
    public SkillResult execute(Map<String, Object> context) {
        ...
        Tool emailTool = toolRegistry.getTool("send_email");
        if (emailTool == null) {
            return SkillResult.failure("Required tool 'send_email' not found");
        }

        Map<String, Object> emailParams = new HashMap<>();
        emailParams.put("to", "user@example.com");
        emailParams.put("subject", "Congratulations!");
        emailParams.put("content", "You have the highest score in our system!");
        
        ToolResult emailResult = emailTool.execute(emailParams);
        toolResults.put("send_email", emailResult);

        if (!emailResult.isSuccess()) {
            return SkillResult.failure("Failed to send email: " + emailResult.getError());
        }

        Map<String, Object> result = new HashMap<>();
        result.put("message", "Congratulation email sent successfully");
